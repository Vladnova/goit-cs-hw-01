...
class Parser:
    ...
    def factor(self):  # <--- Додано метод для обробки чисел і дужок
        """Обробляє цілі числа та вирази в дужках."""
        ...
        if token.type == TokenType.INTEGER:
            ...
            node = self.expr()  # <--- Обчислюємо вираз у дужках
            self.eat(TokenType.RPAREN)
            return node

    def term(self):  # <--- Змінено метод term для обробки множення та ділення
        """Обробляє множення та ділення."""
        node = self.factor()  # <--- Викликає factor() для врахування пріоритету
...
class Interpreter:
    ...
    def visit_BinOp(self, node):
        if node.op.type == TokenType.PLUS:
            ...
            if right_val == 0:  # Захист від ділення на нуль
